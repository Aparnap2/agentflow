...
class EnhancedQueueManager:
    def __init__(self):
        ...
        # Cache for frequent data to reduce queries
        ...
        try:
            if self.redis_token:
                ...
                self.redis = redis.Redis(
                    ...
                    # Optimized connection pool settings for Upstash
                    max_connections=10,  # Limit connections for Upstash free tier
                    socket_timeout=5.0,   # Shorter timeout
            ...
            if self.is_upstash:
                ...
                logger.info("Applying Upstash Redis optimizations")
    ...
    async def create_queue(self, queue_name: str, config: Dict[str, Any] = None) -> bool:
        ...
        try:
            ...
            await self.redis.delete(f"queue:{queue_name}:processing")
            await self.redis.delete(f"queue:{queue_name}:completed")
            await self.redis.delete(f"queue:{queue_name}:failed")
    ...
    async def _worker(self, queue_name: str):
        ...
        while self.active_workers.get(queue_name, False):
            try:
                ...
                task_id = await self.redis.zpopmax(f"queue:{queue_name}")
                
                if not task_id:
                    await asyncio.sleep(0.1)
                    continue
                
                task_id = task_id[0][0]  # Extract task ID from zpopmax result
                ...
                await self.redis.sadd(f"queue:{queue_name}:processing", task_id)
                ...
                await self.redis.srem(f"queue:{queue_name}:processing", task_id)
    ...
    async def _process_delayed_tasks(self, queue_name: str):
        ...
        for task_id, score in ready_tasks:
            # Move to main queue
            await self.redis.zrem(f"queue:{queue_name}:delayed", task_id)
    ...
    async def _batch_processor(self):
        """Background task to process context batches with optimized batching"""
        while True:
            try:
                # Check for batches to process
                batch_id = await self.redis.brpop("context_batches", timeout=1)
                
                if batch_id:
                    batch_id = batch_id[1]  # Extract from brpop result
                    
                    # Get batch data
                    batch_data = await self.redis.hget(f"context_batch:{batch_id}", "data")
                    if batch_data:
                        ...
                        # Mark as processed
                        await self.redis.hset(f"context_batch:{batch_id}", "processed", True)
    ...
    async def _publish_event(self, event_type: str, data: Dict[str, Any]):
        """Publish real-time event to subscribers with enhanced data"""
        ...
        try:
            await self.redis.publish("agentflow_events", json.dumps(event, default=str))
